...
import { getIssuesAcceptedByUser, getIssuesOnUser, getMyAcceptedIssues, getMyPendingIssues, updateIssueChat } from "../../apis/commonAPIs/issueApi.js";
...
const appInitializer = (username, components) => {
    ...
    let btn = new components.PUBLISH_BTN.PublishBtn({contents: [ {type: "issue", icon: "lightbulb"}]});
            ...
            case components.MIDDLE_NAV.middleNavEvents.SELECT_A_SUB_SECTION:{
                ...
                if(state['MiddleNav'].currentSubSection == "/events/attending")
                {
                    ...
                    getAttendingEvents().then((data) => {
                        const eventCollection = new components.EVENT_COLLECTION.EventCollection({pending: data, subSection: "/attending", user_type: 'casual'});
                        element = createComponent(eventCollection);
                        window.__MIDDLE_PANEL__.appendChild(element);
                        window[eventCollection.name] = eventCollection;
                ...
                {
                    window.__MIDDLE_PANEL__.innerText = "";
                    let issue = new components.ISSUE.Issue({subSection : "/accepted", user_type: 'casual'});
                    let element = createComponent(issue);
                    window.__MIDDLE_PANEL__.appendChild(element);
                    window[issue.name] = issue;
                    getMyAcceptedIssues().then((data)=>{
                        let issueCollection = new components.ISSUE_COLLECTION.IssueCollection({pending:data, subSection:"/accepted", user_type: 'casual'});
                ...
                {
                    window.__MIDDLE_PANEL__.innerText = "";
                    const issue = new components.ISSUE.Issue({subSection : "/pending", user_type: 'casual'});
                    let element = createComponent(issue);
                    window.__MIDDLE_PANEL__.appendChild(element);
                    window[issue.name] = issue;
                    getMyPendingIssues().then((data)=>{
                        const issueCollection = new components.ISSUE_COLLECTION.IssueCollection({pending:data, subSection:"/pending", user_type: 'casual'});
                        window[issueCollection.name] = issueCollection;
                        element = createComponent(issueCollection);
                        window.__MIDDLE_PANEL__.appendChild(element);
